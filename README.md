...
侧栏「智能助手」里点 **📖 小钻风餐厅** 进入。这是一个本地 **AI 动态导读阅读器 + 知识图谱**：每天把一篇「小钻风 / Hot from Kitchen」导读 HTML 丢进 `data/reader/decks/`，就能在阅读器里连续滚动阅读、做选区动作、跨天沉淀成一张知识图谱。和手帐共用同一个服务、同一个 Claude CLI（"CC 一次只干一件事"对两边统一生效）。
