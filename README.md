...
侧栏「智能助手」里点 **📖 小钻风阅读器** 进入。这是一个本地 **AI 动态导读阅读器 + 知识图谱**：每天把一篇「小钻风 / Hot from Kitchen」导读 HTML 丢进 `data/reader/decks/`，就能在阅读器里连续滚动阅读、做选区动作、跨天沉淀成一张知识图谱。和手帐共用同一个服务、同一个 Claude CLI（"CC 一次只干一件事"对两边统一生效）。

- **读**：原始 deck 重排成连续滚动长文流（模板无关，保留原设计）。
- **标 / 问**：选区后 ①高亮+笔记 ②打标签 ③术语解释（CC 生成并入图谱）④问 CC ⑤**选中即译**（中文）。
- **图谱**：`/reader/graph` 实体网络 / 时间线 / 观点 / 基于全图问答；跨天 `same_thread_as` 红虚线。
- **学习**：`/reader/learn` 术语表 + CC 出题小测。
- **和手帐耦合**：译文 / 术语 / 问答都能一键 **「存进手帐」**，变成一条 `#小钻风` 便签，自动进看板、摘要、任务图谱。

### 翻译模型（可选，不进安装包）

选中即译默认用本地 **Ollama + Qwen**（快、不走 CC 额度）：

- 装了 [Ollama](https://ollama.com) 并运行、且已下载模型 → 直接用 `qwen2.5:3b`（快译）/ `qwen2.5:7b`（精翻）。
- 装了 Ollama 但没下模型 → 阅读器副栏会出现**一键下载**按钮（约 2GB，一次即可）。
- 没装 Ollama → **自动兜底用 CC 翻译**（较慢、走额度，但零额外安装）。

模型体积大，**不随安装包发布**；是否下载由你决定。可用环境变量 `OLLAMA_MODEL` / `OLLAMA_MODEL_FINE` 自定义。
